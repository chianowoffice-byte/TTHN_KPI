...
function reviewItemHtml(item) {
  const coDiem = CAP_CO_DIEM.includes(item.cap);
...
async function saveChanges(app, onLogout, onGoto) {
  ...
  for (const pid of staged.approve) {
    ...
    const coDiem = card.dataset.coDiem === 'true';
    const tienDo = coDiem ? parseFloat(card.querySelector('.rv-tien-do').value) : null;
    const chatLuong = coDiem ? parseFloat(card.querySelector('.rv-chat-luong').value) : null;
    try {
